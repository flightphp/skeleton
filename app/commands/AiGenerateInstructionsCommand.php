...
class AiGenerateInstructionsCommand extends Command
{
    ...
    public function execute()
    {
        ...
        if (!file_exists($runwayCredsFile)) {
            $io->error('Missing .runway-creds.json. Please run ai:init first.', true);
            return 1;
        }
        ...
        $composerLibs = $io->prompt('What major composer libraries will you be using if you know them right now? (comma separated)', '');
        ...
        $other = $io->prompt('Any other important requirements or context? (optional)', '');
        ...
        if (!is_dir($baseDir . '.github')) {
            mkdir($baseDir . '.github', 0777, true);
        }
